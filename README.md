...
> 发布状态：公开仓已建立，完整运行时、Codex 插件和配置化安装路径均已公开。2026 年 7 月 26 日，目标长期运行实例已完成配置化接管：版本切换后 `readiness=ready`，Doctor 全通过，三个后台角色健康，同一飞书应用只有一个官方消息消费者，冻结后正确进入 `safe-but-disabled` 并可恢复；此前同一运行实现的 Bot/主体用户双身份回复、`🤖` 标识和每日工作记忆闭环证据继续有效。本版本只同步版本资料和最终验收状态，没有运行时业务代码变化；源码完整测试 345 项中 343 项通过、2 项按设计跳过，distribution 94/94，136 个公开文件扫描零发现。
