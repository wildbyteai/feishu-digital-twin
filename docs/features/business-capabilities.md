...
可插拔业务能力让数字分身在普通业务对话中按需读取最新公开信息或部署者批准的内部资料。它不是独立搜索模式，也不会把本机全部 MCP、浏览器或文件开放给 Codex。
...
业务决策 Codex 始终离线。它只看到语义能力、用途、允许操作、风险、信任域、就绪状态和有界输入说明；看不到私有包路径、MCP server reference、工具名、地址或凭据。实际查询由可信运行时通过 `CapabilityGateway` 执行。
...
- 只声明 `read` 风险、`internal` 信任域和 `human-fallback`；
- 精确列出允许工具、只读操作、允许输入字段、必需字段和最大输入字节；
...
`--approve-capability-trust-zone internal` 只批准这次新增或改变的内部数据边界，不是日常开关。能力包改变 MCP server reference、工具绑定、信任域、操作或输入限制时，必须重新 setup 和确认。

这一步安装并验证声明，不会让标准 CLI 扫描或复用用户主 Codex/桌面应用中的 MCP 配置。公共通用 Adapter 通过显式 `resolveCapabilityServer` seam 接收部署者管理的 server；映射工具只有在 `listTools` 元数据明确标记 `annotations.readOnlyHint: true` 且未标记 destructive 时才可用，缺失或含糊时失败关闭。标准 CLI 默认不注入 resolver 或 transport。部署运行组合没有提供获准 resolver 时，Doctor 和查询会稳定显示 `unavailable` 并执行人工兜底，而不会尝试发现其他 MCP。真实内部 MCP 的 server 实现、认证和这段部署接线继续由部署者管理，不进入公共源码、文档示例或发行物。
...
查询出现不可用、未登录、无权限、超时、输入无效、失败或空结果时，不会改用其他信任域、浏览器或本机文件。可信运行时在原授权会话中以建议身份执行 `human-fallback`，明确说明需要人工读取或处理，不猜测业务内容，也不自动联系其他人员。
