...
能力包以 `schema_version: 1` 作为结构兼容边界。旧的纯查询能力包无需增加 `actions`；动作型包可以只声明 `actions`，但查询和动作不能同时为空。未知 schema 失败关闭；`pack_version` 只作为部署者审计和升级标识，任何版本仍要重新通过完整 Schema、工具风险、`internal` 信任域、精确工具白名单、输入限制和确认映射校验。相同 `pack_id` 的 MCP server reference、工具、操作、信任域、确认映射或输入边界发生变化时，必须通过 setup 重新提供 manifest，并重新确认对应信任域。

`reuse_codex_mcp_servers: true` 只允许运行时对已安装能力包中的精确 server reference 执行 `codex mcp get`，不列举或复用其他本机 MCP。MCP `tools/list` 必须明确证明查询工具为只读、准备工具为非破坏性、提交工具为破坏性，否则相应能力显示 `unavailable`。
